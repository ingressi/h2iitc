...
* 우하단에 고배율 지도 mini map
  * (참고로, 이 다양한 control들은 로딩상황상 순서나 위치가 서로 뒤바뀔 수 있음)
* 약 1cm남짓 척도의 화면 실제 거리 scale bar
  * 선택된 바탕 지도 map tile과의 궁합에 따라 스케일바가 사라지기도 함.
...
* scale bar : 지도의 실제 거리를 알 수 있는 축적자 표출.
  * 선택된 map tile과의 궁합에 따라 스케일바가 안 보이기도 함.
...
참고로 선택 사용하는 map tile 종류와의 궁합에 따라, 지도에 띄워 쓰는
plugin control 중에 "scale bar"가 안보이게 되기도 합니다.
...
<img src="IITC_53.jpg" />  
포탈 이름을 눌러 2개 혹은 3개를 선택(녹색)한 뒤, Draw& 버튼을 눌러서 링크 및 
필드를 구상해볼 수 있습니다. (참고로, OK 버튼은 기능이 없어서 누를 일이 없음.)  
<img src="IITC_54.jpg" />  
